...
class ObjectiveCV(ABC):
    ...
    def __init__(self, X, y, pipe=None, seed=1, eps=1e-15, n_splits=5, n_class=None):
        self._X = X
        self._y = y
        self._pipe = pipe
        self._seed = seed
...
class CmdFactory:
    ...
    def hpt(self, train_data, space, trials_file, max_iter, steps, seed,
            pipe=None, n_class=None):
        X, y = train_data
        cv = self._cls(X, y, seed=seed, pipe=pipe, n_class=n_class)

        resume_trials(cv, space, trials_file, max_iter, trials_step=steps)

    def pred(self, train_data, test_data, params, out_tr, out_test, n_bags, seed,
             pipe=None, n_class=None):
        X, y = train_data
        X_test = test_data

        def pred_nth_bagging(n):
            cv = self._cls(X, y, seed=seed + n, pipe=pipe, n_class=n_class)
